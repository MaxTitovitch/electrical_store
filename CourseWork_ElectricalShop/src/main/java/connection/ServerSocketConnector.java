package connection;

import logging.LoggerSaver;
import model.*;
import runing.ServerRunner;

import java.io.*;
import java.net.Socket;

public class ServerSocketConnector extends Thread{
    private ServerRunner parental = null;
    private Socket clientAccepted;
    private ObjectInputStream sois = null;
    private ObjectOutputStream soos = null;
    private boolean isExit = false;
    private boolean isExitWitchServer = false;
    private boolean isBadExit = false;
    public boolean isHaveError = false;
    public String errorMessage = "";
    private int number;


    public ServerSocketConnector(Socket clientAccepted, int number, ServerRunner parental, ObjectInputStream sois, ObjectOutputStream soos) {
        this.clientAccepted = clientAccepted;
        this.number = number;
        this.parental = parental;
        this.sois = sois;
        this.soos = soos;
        start();
    }

    public void run(){
        try {
            //soos = new ObjectOutputStream(clientAccepted.getOutputStream());
            while (!isExit) {
                jobWithClient();
                clearLastError();
            }
            if(!isBadExit) {
                if (isExitWitchServer) exitWitchServer();
                else exit();
            }
            interrupt();
        } catch (Exception e){
            LoggerSaver.logger.error("Ошибка соединения с клиентом #" + number + ": " +  e.getMessage());
            generateError("Ошибка соединения с клиентом", e.getMessage());
        }
    }


    private void jobWithClient(){
        try {
            String command = sois.readObject().toString();
            String data = sois.readObject().toString();
            String answer = runCommand(command, data);
            LoggerSaver.logger.info("Запрос от клиента #" + number + ": \"" +  command + "\" обработан");
            clearLastError();

            if(!answer.equals("exit"))
                soos.writeObject(answer);
        } catch (IOException | ClassNotFoundException  e){
            LoggerSaver.logger.warn("Ошибка передачи информации серверу от клиента #" + number + ": " +  e.getMessage());
            isExit = true;  isBadExit = true;
            generateError("Ошибка передачи информации серверу", e.getMessage() );
        }
    }

    private String runCommand(String command, String data){
        switch (command){
            case "authorization": return OrderPerformer.doAuthorization(data);
            case "getCategorys": return OrderPerformer.getAll(data, CategoryModel.class);
            case "getProducts": return OrderPerformer.getAll(data, ProductModel.class);
            case "getPurchases": return OrderPerformer.getAll(data, PurchaseModel.class);
            case "getSuppliers": return OrderPerformer.getAll(data, SupplierModel.class);
            case "getUsers": return OrderPerformer.getAll(data, UserModel.class);
            case "getWarehouses": return OrderPerformer.getAll(data, WarehouseModel.class);
            case "getSuppliersProducts": return OrderPerformer.getAll(data, SupplierProductModel.class);
            case "getUserCategorys": return OrderPerformer.getAll(data, UserCategoryModel.class);
            case "delete": return OrderPerformer.delete(data);
            case "insert_categorys": return OrderPerformer.insert(data, "categorys", CategoryModel.class);
            case "insert_products": return OrderPerformer.insert(data, "products", ProductModel.class);
            case "insert_purchases": return OrderPerformer.insert(data, "purchases", PurchaseModel.class);
            case "insert_suppliers": return OrderPerformer.insert(data, "suppliers", SupplierModel.class);
            case "insert_users": return OrderPerformer.insert(data, "users", UserModel.class);
            case "insert_warehouses": return OrderPerformer.insert(data, "warehouses", WarehouseModel.class);
            case "insert_supplie_products": return OrderPerformer.insert(data, "supplie_products", SupplierProductModel.class);
            case "insert_user_categories": return OrderPerformer.insert(data, "user_categories", UserCategoryModel.class);
            case "update_categorys": return OrderPerformer.update(data, "categorys", CategoryModel.class);
            case "update_products": return OrderPerformer.update(data, "products", ProductModel.class);
            case "update_purchases": return OrderPerformer.update(data, "purchases", PurchaseModel.class);
            case "update_suppliers": return OrderPerformer.update(data, "suppliers", SupplierModel.class);
            case "update_users": return OrderPerformer.update(data, "users", UserModel.class);
            case "update_warehouses": return OrderPerformer.update(data, "warehouses", WarehouseModel.class);
            case "update_supplie_products": return OrderPerformer.update(data, "supplie_products", SupplierProductModel.class);
            case "update_user_categories": return OrderPerformer.update(data, "user_categories", UserCategoryModel.class);
            case "exit_witchout_server":  isExit = true;     return "exit";
            case "exit_witch_server": isExit = true;  isExitWitchServer = true;   return "exit";
            default: return "";
        }
    }


    public void exitWitchServer(){
        exit();
        parental.dialog.dispose();
        parental.exitServer();
    }

    public void exit(){

        parental.dialog.textFields.get(1).setText(--parental.count + " шт.");
        try {
            sois.close();
            soos.close();
            clientAccepted.close();
            parental.clientSockets.remove(this);
            clearLastError();
            LoggerSaver.logger.info("Отключился клиент: #" + number);
        } catch (IOException e) {
            generateError("Ошибка закрытия соедитения с клиентом", e.getMessage());
            LoggerSaver.logger.error("Ошибка закрытия соедитения с клиентом #" + number + ": " +  e.getMessage());
        }
    }


    private void generateError(String message, String javaMessage){
        isHaveError = true;
        errorMessage = message + ". Ошибка: \"" + javaMessage +"\"";
    }

    private void clearLastError(){
        isHaveError = false;
        errorMessage = "";
    }


}
